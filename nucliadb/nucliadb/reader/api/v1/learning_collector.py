...
@api.get(
    path=f"/{KB_PREFIX}/{{kbid}}/learning/collect/feedback/{{month}}",
    status_code=200,
...
async def feedback_download(
    ...
    return await learning_collector.proxy(
        request, "GET", f"/collect/feedback/{kbid}/{month}"
    )


@api.get(
    path=f"/{KB_PREFIX}/{{kbid}}/learning/collect/feedback",
    status_code=200,
...
async def feedback_list_months(
    ...
    return await learning_collector.proxy(request, "GET", f"/collect/feedback/{kbid}")
