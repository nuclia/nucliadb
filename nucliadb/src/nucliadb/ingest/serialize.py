    ...
    if field_type_filter and (include_values or include_extracted_data):
        ...
        for (field_type, field_id), field in orm_resource.fields.items():
            ...
            if field_type_name is FieldTypeName.TEXT:
                ...
                if include_value:
                    resource.data.layouts[field.id].value = models.FieldLayout.from_message(
                        value  # type: ignore
                    )
                if include_errors:
                    error = await field.get_error()
                    if error is not None:
                        resource.data.layouts[field.id].error = Error(body=error.error, code=error.code)
                if include_extracted_data:
                    resource.data.layouts[field.id].extracted = LayoutFieldExtractedData()
                    await set_resource_field_extracted_data(
                        field,
                ...
                if include_value:
                    resource.data.datetimes[field.id].value = models.FieldDatetime.from_message(
                        value  # type: ignore
                    )
                if include_extracted_data:
                    resource.data.datetimes[field.id].extracted = DatetimeFieldExtractedData()
                    await set_resource_field_extracted_data(
                        field,
                ...
                if include_value:
                    resource.data.keywordsets[field.id].value = models.FieldKeywordset.from_message(
                        value  # type: ignore
                    )
                if include_extracted_data:
                    resource.data.keywordsets[field.id].extracted = KeywordsetFieldExtractedData()
                    await set_resource_field_extracted_data(
                        field,
