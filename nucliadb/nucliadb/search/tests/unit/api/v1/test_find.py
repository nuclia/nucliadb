...
from nucliadb.search.api.v1.find import find_
...
def create_find_response_mock():
    with mock.patch(
        "nucliadb.search.api.v1.find.create_find_response",
    ) as mocked:
        yield mocked
...
async def test_find_endpoint_handles_errors(
    create_find_response_mock, predict_error, http_error_response
):
    create_find_response_mock.side_effect = predict_error
    ...
    response = await find_knowledgebox_endpoint(
        request=request,
        kbid="kbid",
        item=Mock(),
        x_ndb_client=None,
