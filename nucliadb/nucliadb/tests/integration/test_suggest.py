...
async def test_suggest_paragraphs(
    ...
    # exact match
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=Nietzche")
    ...
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=princes")
    assert resp.status_code == 200
    body = resp.json()
    ...
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=z")
    assert resp.status_code == 200
    body = resp.json()
    assert len(body["paragraphs"]["results"]) == 1
    assert body["paragraphs"]["results"][0]["rid"] == rid2
    ...
    resp = await nucliadb_reader.get(
        f"/v1/kb/{knowledgebox}/suggest?query=Hanna+Adrent"
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_suggest_related_entities(
    ...
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=Ann")
    assert resp.status_code == 200
    body = resp.json()
    assert_expected_entities(body, {"Anna", "Anthony"})

    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=joh")
    assert resp.status_code == 200
    body = resp.json()
    assert_expected_entities(body, {"John"})

    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=xxxxx")
    assert resp.status_code == 200
    body = resp.json()
    ...
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=bar")
    assert resp.status_code == 200
    body = resp.json()
    ...
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=BÄR")
    assert resp.status_code == 200
    body = resp.json()
    assert_expected_entities(body, {"Barcelona", "Bárcenas"})

    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=BáR")
    assert resp.status_code == 200
    body = resp.json()
    assert_expected_entities(body, {"Barcelona", "Bárcenas"})

    # Test multiple word suggest and ordering
    resp = await nucliadb_reader.get(f"/v1/kb/{knowledgebox}/suggest?query=Solomon+Is")
    assert resp.status_code == 200
    body = resp.json()
...
async def test_suggestion_on_link_computed_titles_sc6088(
    ...
    resp = await nucliadb_reader.get(
        f"/v1/kb/{kbid}/suggest", params={"query": "MyLink", "fields": "a/title"}
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_suggest_features(
    ...
    resp = await nucliadb_reader.get(
        f"/v1/kb/{knowledgebox}/suggest",
        params={"query": "ann", "features": ["entities"]},
    )
    assert resp.status_code == 200
    body = resp.json()
    assert_expected_entities(body)
