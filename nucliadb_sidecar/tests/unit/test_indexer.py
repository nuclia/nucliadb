...
class TestConcurrentShardIndexer:
    ...
    def message_progress_updater(self):
        ...
        with patch("nucliadb_sidecar.indexer.MessageProgressUpdater", new=mpu):
            yield mpu
    ...
    async def test_creates_an_indexer_and_a_task_per_shard(
        ...
        with (
            patch("nucliadb_sidecar.indexer.asyncio") as asyncio_mock,
            patch("nucliadb_sidecar.indexer.PriorityIndexer") as PriorityIndexer_mock,
        ):
            seqid = 1
            n_tasks = 5
...
class TestPriorityIndexer:
    ...
    def successful_indexing(self):
        mock = AsyncMock()
        with patch("nucliadb_sidecar.signals.successful_indexing", new=mock):
            yield mock
