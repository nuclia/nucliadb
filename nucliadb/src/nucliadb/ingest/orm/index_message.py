    ...
    for bm in broker_messages:
        for etw in bm.extracted_text:
            if not (etw.field.field_type == FieldType.CONVERSATION and etw.field.field == fieldid.field):
                continue
            splits.update(set(etw.body.split_text.keys()))
        for evw in bm.field_vectors:
            if not (evw.field.field_type == FieldType.CONVERSATION and evw.field.field == fieldid.field):
                continue
            splits.update(set(evw.vectors.split_vectors.keys()))
    return splits
