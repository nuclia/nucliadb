...

{
    "triplets": [
        ...
        {
ede35600b7aa68bc4e952f42c3637ef8f1bd7fa5            "head_entity": "John Adams Roofing Inc.",
            "relationship": "was founded by",
            "tail_entity": "John Adams",
            "score": 5
        },
...
async def fuzzy_search_entities(
    ...
    for word in query.split():
        ...
        node.value = word
        node.type = None
        node.match = NodeMatchKindName.FUZZY_WORDS
        operands.append(node)
    request = GraphNodesSearchRequest(
    ...
    try:
        results = await rpc.graph_nodes(kbid, request)
    except Exception as exc:
        capture_exception(exc)
        logger.exception("Error in finding entities in query for graph strategy")
        return None

    # Convert results to our type and dedup in case there's any duplicate node
    # REVIEW: can the API guarantee no duplicates?
    unique_entities: set[RelatedEntity] = {
        RelatedEntity(family=node.group, value=node.value) for node in results.nodes
    }
