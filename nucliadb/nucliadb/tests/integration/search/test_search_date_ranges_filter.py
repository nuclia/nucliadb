...
def broker_resource_with_dates(kbid: str) -> BrokerMessage:
    bmb = BrokerMessageBuilder(
        kbid=kbid, rid=str(uuid4()), vector_dimension=VECTOR_DIMENSION
    )
    bmb.fields.clear()
    bmb.with_text_field("text1", "My text")
    bmb.bm.origin.ClearField("created")
    bmb.bm.origin.ClearField("modified")
    return bmb.build()


@pytest.fixture(scope="function")
async def resource(nucliadb_grpc, knowledgebox):
    bm = broker_resource_with_dates(knowledgebox)
...
@pytest.mark.parametrize("knowledgebox", ("STABLE",), indirect=True)
