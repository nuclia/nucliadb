...
use nucliadb_core::{query_language::BooleanExpression, NodeResult};
use nucliadb_vectors::{
    config::VectorConfig,
    data_point::{create, DataPointPin, Elem, LabelDictionary, NoDLog},
    formula::{AtomClause, Clause, CompoundClause, Formula},
};
...
fn test_recall_clustered_data() -> NodeResult<()> {
            ...
            .map(|(i, (k, v))| {
                Elem::new(
                    ...
                    if i == 33 {
                        ...
                        LabelDictionary::new(vec!["/q/h".to_string()])
                    },
                    None,
                )
            })
    ...
    let t = Instant::now();
    let results: Vec<_> = dp.search(&NoDLog, &query, &f, false, 50, &config, -1.0).collect();
    println!("Found {} results in {:?}", results.len(), t.elapsed());

    // Search with hidden filter
    f.extend(CompoundClause::new(
        nucliadb_vectors::formula::BooleanOperator::Not,
        vec![Clause::Atom(AtomClause::label("/q/h".to_string()))],
    ));
    let t = Instant::now();
    let results: Vec<_> = dp.search(&NoDLog, &query, &f, false, 5, &config, -1.0).collect();
    println!("Found {} results in {:?}", results.len(), t.elapsed());
