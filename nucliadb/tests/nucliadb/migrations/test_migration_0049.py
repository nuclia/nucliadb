...
migration: Migration = get_migration(49)


async def test_migration_0049(maindb_driver: Driver):
    ...
    async with maindb_driver.rw_transaction() as txn:
        find: SearchConfiguration = FindSearchConfiguration(
            kind="find", config=FindConfig(generative_model="azure-deepseek-r1")
        )
        ...
        ask: SearchConfiguration = AskSearchConfiguration(
            kind="ask", config=AskConfig(generative_model="azure-deepseek-r1")
        )
    ...
    async with maindb_driver.ro_transaction() as txn:
        ...
        assert find_.config.generative_model == "chatgpt-azure-5.6-luna"  # type: ignore[attr-defined]

        ask_ = await datamanagers.search_configurations.get(txn, kbid=kbid, name="ask")
        assert ask_ is not None
        assert ask_.config.generative_model == "chatgpt-azure-5.6-luna"  # type: ignore[attr-defined]
