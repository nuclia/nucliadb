    ...
    fn index_paragraph(&mut self, resource: &Resource) -> tantivy::Result<()> {
        ...
        if let Some(security_groups) = resource_security.filter(|i| !i.access_groups.is_empty()) {
            for group_id in security_groups.access_groups.iter() {
                ...
                security_facets.push(facet.clone());
            }
        }
        ...
        for (field, text_info) in &resource.texts {
            ...
            for (paragraph_id, p) in inspect_paragraph(field) {
                ...
                // Add security facets to each paragraph
                if security_facets.is_empty() {
                    doc.add_u64(self.schema.groups_public, 1_u64);
                } else {
                    doc.add_u64(self.schema.groups_public, 0_u64);
                    for facet in security_facets.iter() {
                        doc.add_facet(self.schema.groups_with_access, facet.clone());
                    }
                }
