...
async def test_create_shadow_shard(grpc_servicer, fake_node):
    ...
    rep2_id, rep2_node = replica2.shard.id, replica2.node

    # Create a replica of rep1 onto the other node
    req = ShadowShardCreateRequest(kbid=kbid, replica_id=rep1_id, node=rep2_node)
    resp = await stub.ShadowShardCreate(req)
