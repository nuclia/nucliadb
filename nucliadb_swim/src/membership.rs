...
impl ArtilleryMemberList {
    ...
    pub fn find_member_by_addr(&self, addr: &SocketAddr) -> Option<ArtilleryMember> {
        let chosen_position: Option<usize> =
            self.members
                .iter()
                .position(|member| match member.remote_host() {
                    Some(remote_addr) => remote_addr == *addr,
                    None => false,
                });
        chosen_position.map(|chosen_position| self.members[chosen_position].clone())
    }
...
mod tests {
    ...
    #[test]
    fn membership_test() {
        let current = ArtilleryMember::current("myid".to_string(), 'N');
