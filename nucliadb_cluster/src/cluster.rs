...
use chitchat::{spawn_chitchat, ChitchatConfig, NodeId, FailureDetectorConfig, ChitchatHandle};
...
impl fmt::Display for NucliaDBNodeType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self{
            NucliaDBNodeType::Reader => write!(f, "Reader"),
            NucliaDBNodeType::Writer => write!(f, "Writer"),
...
impl FromStr for NucliaDBNodeType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s{
            "Reader" => Ok(NucliaDBNodeType::Reader),
            "Writer" => Ok(NucliaDBNodeType::Writer),
            "Node" => Ok(NucliaDBNodeType::Node),
            _ => Err(ClusterError::ParseNodeTypeError { message: "can't parse node type string".to_string() })
        }
    }
}
...
impl Cluster {
    ...
    pub async fn new(
        ...
        let chitchat_handle = spawn_chitchat(config, Vec::new(), &UdpTransport).await.unwrap();
        let (self_id, mut cluster_watcher) = chitchat_handle.with_chitchat(|chitchat|{
            let state= chitchat.self_node_state();
            state.set("node_type", node_type.clone());
            (chitchat.self_node_id().clone(), chitchat.live_nodes_watcher())
        }).await;
        ...
        tokio::task::spawn(async move{
            while !task_stop.load(Ordering::Relaxed) {
                if let Some(live_nodes) = cluster_watcher.next().await {
                    ...
                    let update_members: Vec<Member> = 
                        live_nodes.iter()
                        .map(|node_id| {
                            let state = chitchat.node_state(node_id).unwrap();
                            Member {
                                node_id : node_id.id.clone(),
                                node_type: NucliaDBNodeType::from_str(state.get("node_type").unwrap()).unwrap(),
    ...
    pub async fn members(&self) -> Vec<Member> {
        let mut nodes: Vec<Member> = self.chitchat_handle.with_chitchat(|chitchat| {
