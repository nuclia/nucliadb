...
from nucliadb.common.models_utils.from_proto import extra_from_proto, origin_from_proto
    ...
    async def _get_origin(kbid: str, rid: str) -> tuple[str, Optional[Origin]]:
        ...
        if resource is not None:
            ...
            if pb_origin is not None:
                origin = origin_from_proto(pb_origin)
        return rid, origin
    ...
    async def _get_extra(kbid: str, rid: str) -> tuple[str, Optional[Extra]]:
        ...
        if resource is not None:
            ...
            if pb_extra is not None:
                extra = extra_from_proto(pb_extra)
        return rid, extra
