...
use clap::Parser;
use nucliadb_vectors2::hnsw::Hnsw;
use tempfile::*;
...
fn main() {
    ...
    match (&args.path, ouput) {
        (Some(path), Some(output)) => match analyze_path(path) {
            Some(path) => nucliadb_vectors2::sic::checks(path.as_path(), output.as_path()),
            None => println!("Invalid path {path}"),
        },
        (None, Some(output)) => {
            let temp_dir = tempfile::tempdir().unwrap();
            writer_process(&temp_dir, &args);
            nucliadb_vectors2::sic::checks(temp_dir.path(), output.as_path());
        }
