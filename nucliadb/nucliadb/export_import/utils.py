...
class IteratorExportStream(ExportStream):
    """
    Adapts the parent class to be able to read bytes yielded from an async iterator of bytes.
    """
...
class ExportStreamReader:
    ...
    @classmethod
    def from_iterator(cls, iterator: AsyncIterator[bytes]):
        return cls(IteratorExportStream(iterator))
    ...
    async def maybe_read_learning_config(
        ...
        """
        Tries to read a learning config from the stream. If not found, returns None.
        It also returns the bytes that were read from the stream but not parsed.
        """
        type_bytes = await self.stream.read(3)
...
def stream_compatible_with_kb(
    ...
    async def wrapped() -> AsyncGenerator[bytes, None]:
        chunk = await _check_semantic_model_compatibility(kbid, stream)
        yield chunk
...
async def _check_semantic_model_compatibility(
    ...
    stream_reader = ExportStreamReader.from_iterator(stream.__aiter__())
    lconfig, read_bytes = await stream_reader.maybe_read_learning_config()
    if lconfig is None:
        logger.warning(
