...
async def should_backfill_kb(kbid: str) -> bool:
    async with datamanagers.with_ro_transaction() as txn:
        ...
        async for rid in resources_v1.iter(kbid=kbid):
            if "-" in rid:
        ...
        all_resources_v2 = {rid async for rid in resources_v2.iter(kbid=kbid)}
...
async def backfill_kb(*, kbid: str) -> None:
    ...
    while True:
        ...
        v2_rids: set[str] = set()
        async for rid in resources_v2.iter(kbid=kbid):
            v2_rids.add(rid)
...
async def _backfill_resource_in_txn(txn: Transaction, *, kbid: str, rid: str) -> None:
    ...
    shard = await resources_v1.get_shard(txn, kbid=kbid, rid=rid)
    if shard is None:
...
class resources_v1:
    ...
    @classmethod
    async def iter(cls, *, kbid: str) -> AsyncGenerator[str, None]:
        """
