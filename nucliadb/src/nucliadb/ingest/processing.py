...
class PushPayload(BaseModel):
    ...
    # New File
    # file fields are pushed to the storage and its metadata is serialized as jwt tokens
...
class ProcessingEngine:
    ...
    def generate_file_token_from_fieldfile(self, file: FieldFilePB) -> str:
        ...
        expiration = now + datetime.timedelta(days=self.days_to_keep)
        processing_options = {}
        if file.processing_options:
            processing_options = MessageToDict(
    ...
    async def convert_internal_filefield_to_str(self, file: FieldFilePB, storage: Storage) -> str:
        if self.onprem is False:
            # For hosted, at this point the binary has already been uploaded to the storage. We simply need to
            # generate a token for the file with all the necessary metadata so that processing can access it.
            jwttoken = self.generate_file_token_from_fieldfile(file)
        else:
            # For on-prem, we need to upload the file to Nuclia so that it can be processed.
            # The token is generated by Nuclia and returned in the response. This is passed back
            # to processing.
            headers = {}
            headers["X-PASSWORD"] = file.password
            headers["X-LANGUAGE"] = file.language
