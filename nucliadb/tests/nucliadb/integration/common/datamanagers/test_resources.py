...
async def resource_with_slug(maindb_driver: Driver, kbid: str):
    ...
    async with maindb_driver.rw_transaction() as txn:
        await datamanagers.resources.set(txn, kbid=kbid, rid=rid, basic=Basic(slug=slug))
...
async def test_all_fields(maindb_driver: Driver, resource_with_slug: tuple[str, str, str]):
    ...
    async with maindb_driver.ro_transaction() as txn:
        ...
        assert (await datamanagers.fields.exists(txn, kbid=kbid, rid=rid, field_id=field)) is False
    ...
    async with maindb_driver.ro_transaction() as txn:
        ...
        assert (await datamanagers.fields.exists(txn, kbid=kbid, rid=rid, field_id=field)) is True
