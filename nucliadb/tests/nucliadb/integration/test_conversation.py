...
async def test_reprocess_conversation_field_sends_generated_conversation_to_processing(
    ...
    resp = await nucliadb_writer.post(
        f"/kb/{kbid}/resources",
        json={
            "slug": f"test-resource-{random.randint(0, 100000)}",
            "title": "Test Resource",
            "conversations": {
                source_conv_id: {
                    "messages": [
    ...
    # Reprocess the source conversation field and check that generated conversation is sent to processing too.
    processing = get_processing()
    assert isinstance(processing, DummyProcessingEngine)
    processing.calls.clear()
    processing.values.clear()
    ...
    resp = await nucliadb_writer.post(
        f"/kb/{kbid}/resource/{rid}/conversation/{source_conv_id}/reprocess",
    )
    assert resp.status_code == 202
