...
class IndexMessageBuilder:
    ...
    async def _apply_resource_level(self, brain: ResourceBrain) -> None:
    ...
    async def for_writer_bm(
        ...
        await self._apply_resource_level(self.brain)
        ...
        for fieldid in fields_to_index:
            if is_deleted_field(fieldid, messages):
                continue
            await self._apply_field_level(
                self.brain,
                fieldid,
                basic,
...
def needs_relations_update(
    ...
    return (
        has_new_field_metadata(field_id, messages)
        or has_new_extracted_text(field_id, messages)
        or has_token_annotations(field_id, messages)
    )


def has_token_annotations(
    field_id: FieldID,
    messages: list[BrokerMessage],
) -> bool:
    for message in messages:
...
def is_deleted_field(
    field_id: FieldID,
    messages: list[BrokerMessage],
) -> bool:
    return field_id in get_bm_deleted_fields(messages)
