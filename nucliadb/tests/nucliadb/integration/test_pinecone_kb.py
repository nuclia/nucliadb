...
def mock_pinecone_client():
    ...
    with unittest.mock.patch(
        "nucliadb.ingest.orm.knowledgebox.get_pinecone_client", return_value=pinecone_mock
    ):
        yield pinecone_mock
...
async def test_kb_creation(
    ...
    config = KnowledgeBoxConfig(
        ...
        external_index_provider=ExternalIndexProvider(
            type=ExternalIndexProvider.ProviderType.PINECONE,
            config={
    ...
    mock_pinecone_client.create_index.assert_awaited_once_with(name=kbid, dimension=128)
    ...
    async with datamanagers.with_ro_transaction() as txn:
        ...
        assert external_index_provider.type == ExternalIndexProvider.ProviderType.PINECONE
        # Check that the API key was encrypted
        assert "api_key" not in external_index_provider.config
        encrypted_api_key = external_index_provider.config["encrypted_api_key"]
        endecryptor = get_endecryptor()
        decrypted_api_key = await endecryptor.decrypt_text(encrypted_api_key)
        assert decrypted_api_key == "my-pinecone-api-key"

        # Check that the rest of the config was stored
        assert external_index_provider.config["index_name"] == kbid
        assert external_index_provider.config["index_host"] == "pinecone-host"
    ...
    mock_pinecone_client.delete_index.assert_awaited_once_with(name=kbid)
