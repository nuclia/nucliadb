...
class Resource:
    ...
    async def get_basic(self) -> PBBasic:
        data = await self.get_data(columns=("basic",))
        if data is None or data.basic is None:
            return PBBasic()
        return data.basic

    async def get_origin(self) -> PBOrigin | None:
        data = await self.get_data(columns=("origin",))
        return data.origin if data is not None else None

    async def get_extra(self) -> PBExtra | None:
        data = await self.get_data(columns=("extra",))
        return data.extra if data is not None else None

    async def get_security(self) -> utils_pb2.Security | None:
        data = await self.get_data(columns=("security",))
        return data.security if data is not None else None
