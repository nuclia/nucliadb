use dockertest::{DockerTest, Composition, StartPolicy};
use nucliadb_cluster::cluster::{Cluster, NucliaDBNodeType, Member, CLUSTER_GOSSIP_INTERVAL};
...
use std::{io, env, net::{SocketAddr, IpAddr, Ipv4Addr, TcpListener}, str::FromStr, sync::atomic::AtomicUsize};
use log::error;
...
pub async fn create_cluster_for_test_with_id(peer_uuid: String, seed_node: String) -> anyhow::Result<Cluster> {
    let port = find_available_port()?;
    eprintln!("port: {port}");
    let peer_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), port);
    let cluster = Cluster::new(peer_uuid, peer_addr, NucliaDBNodeType::Node, vec![seed_node]).await?;
    Ok(cluster)
}
...
async fn test_cluster_two_nodes() {
    ...
    // add node to cluster
    let cluster1= create_cluster_for_test(SEED_NODE.to_string()).await.unwrap();
        ...
        loop{
            if let Ok(_) = unix_stream.readable().await {
                ...
                match unix_stream.try_read(&mut buffer) {
                    Ok(bytes_read) => {
                        assert_ne!(bytes_read, 0, "0 bytes read from socket. Connection closed by writer");
                        ...
                        if state.nodes() == 2 {
                            ...
                            break //TODO: when dockertest crate will implement stop/pause delete this break
                        }
                    },
