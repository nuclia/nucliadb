...
async fn cluster_creator(node_id: String, listen_addr: SocketAddr, node_type: String, rx_command: Receiver<ClusterCommands>, tx_members: Sender<Vec<RustMember>>) -> anyhow::Result<()> {
    info!("Starting ChitChat cluster");
    let cluster = RustCluster::new(node_id,
            listen_addr,
    ...
    loop {
        ...
        match command {
            ...
            ClusterCommands::AddPeerNode(addr) => {
                ...
                cluster.add_peer_node(addr).expect("Error during send Syn message to peer");
            }
        }
    }
}
...
impl Cluster {
    ...
    fn new(
        ...
        let t_handle = std::thread::spawn(move || {
            ...
            if let Err(e) = rt.block_on(cluster_creator(node_id, listen_addr, node_type, rx_command, tx_members)) {
                error!("{e}");
                return
            }
        });
