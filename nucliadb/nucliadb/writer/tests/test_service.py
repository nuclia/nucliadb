...
async def test_service_lifecycle_entities(writer_api, entities_manager_mock):
    ...
    async with writer_api(roles=[NucliaDBRoles.WRITER]) as client:
        ...
        resp = await client.post(f"/v1/kb/{kbid}/entitiesgroups", json=eg.dict())
...
async def test_service_lifecycle_labels(writer_api):
    ...
    async with writer_api(roles=[NucliaDBRoles.WRITER]) as client:
        ...
        ls.labels.append(Label(title="asd"))
        resp = await client.post(f"/v1/kb/{kbid}/labelset/ls1", json=ls.dict())
        assert resp.status_code == 200
        resp = await client.post(f"/v1/kb/{kbid}/labelset/ls2", json=ls.dict())
        assert resp.status_code == 200
