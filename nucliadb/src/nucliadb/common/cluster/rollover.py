...
async def schedule_resource_indexing(app_context: ApplicationContext, kbid: str) -> None:
    ...
    batch = []
    async for resource_id in datamanagers.resources.iterate_ids(kbid=kbid):
...
async def _index_resource_to_rollover_index(
    ...
    async with resource_index_semaphore:
        async with datamanagers.with_transaction() as txn:
            shard_id = await datamanagers.resources.get_shard_id(txn, kbid=kbid, rid=resource_id)
        if shard_id is None:
            logger.warning(
...
async def validate_indexed_data(
    ...
    async for resource_id in datamanagers.resources.iterate_ids(kbid=kbid):
        ...
        if indexed_data is not None:
            ...
            async with datamanagers.with_transaction() as txn:
                shard_id = await datamanagers.resources.get_shard_id(  # type: ignore[assignment]
