...
impl ShardReader {
    ...
    pub fn suggest(&self, request: SuggestRequest) -> NodeResult<SuggestResponse> {
        ...
        //     let (paragraphs, entities) = crossbeam_thread::join(paragraph_task, relation_task);
        //     let paragraphs = paragraphs?;
        //     (Some(paragraphs), Some(entities))
        // } else if suggest_paragraphs {

        // }

        // let paragraph_reader_service = self.paragraph_reader.clone();
        // let paragraph_task = if suggest_paragraphs {
        //     let info = info_span!(parent: &span, "paragraph suggest");
        //     let paragraph_task = move || Some(paragraph_reader_service.suggest(&request));
        //     let paragraph_task = || run_with_telemetry(info, paragraph_task);
        //     paragraph_task
        // };

        // let relations_reader_service = self.relation_reader.clone();
        // let prefixes = Self::split_suggest_query(request.body.clone(),
        // MAX_SUGGEST_COMPOUND_WORDS); let relation_task = if suggest_entities {
        //     let relation_searches = prefixes
        //         .par_iter()
        ...
        //     let info = info_span!(parent: &span, "relations suggest");
        //     let relation_task = move || Some(relation_searches.collect::<Vec<_>>());
        //     let relation_task = || run_with_telemetry(info, relation_task);
        //     relation_task
        // };
        ...
        // crossbeam_thread::scope(|s| {
        //     if let Some(paragraph_task) = paragraph_task {
        //         s.spawn(|_| paragraphs = paragraph_task());
        //     }
        //     if let Some(relation_task) = relation_task {
        //         s.spawn(|_| entities = relation_task());
        //     }
        // })
        // .expect("Failed to join threads");
