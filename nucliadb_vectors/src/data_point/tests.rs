...
use crate::data_point::{DeleteLog, Elem, LabelDictionary, OpenDataPoint, Similarity};
...
fn simple_flow() {
    ...
    let reader = OpenDataPoint::new(temp_dir.path(), elems, None, SIMILARITY).unwrap();
    let id = reader.get_id();
    let reader = OpenDataPoint::open(temp_dir.path(), id).unwrap();
...
fn single_graph() {
    ...
    let reader = OpenDataPoint::new(temp_dir.path(), elems.clone(), None, SIMILARITY).unwrap();
...
fn data_merge() {
    ...
    let dp_0 = OpenDataPoint::new(temp_dir.path(), elems0, None, SIMILARITY).unwrap();
    let dp_1 = OpenDataPoint::new(temp_dir.path(), elems1, None, SIMILARITY).unwrap();
    let work = &[(HashSet::default(), dp_1.get_id()), (HashSet::default(), dp_0.get_id())];
    let dp = OpenDataPoint::merge(temp_dir.path(), work, Similarity::Cosine).unwrap();
    let formula = Formula::new();
