...
@pytest.mark.xfail(RUNNING_IN_GH_ACTIONS, reason="Somethimes this fails in GH actions")
@pytest.mark.asyncio
async def test_multiple_find_resource_all(
    search_api: Callable[..., AsyncClient], multiple_search_resource: str
) -> None:
    kbid = multiple_search_resource
    ...
    async with search_api(roles=[NucliaDBRoles.READER]) as client:
        ...
        resp = await client.get(
            f"/{KB_PREFIX}/{kbid}/find?query=own+text&highlight=true&page_number=0&page_size=40",
        )
        if resp.status_code != 200:
            print(resp.content)
        ...
        pdb.set_trace()
        assert resp.status_code == 200
        assert len(resp.json()["resources"]) == 40
...
async def test_search_resource_all(
    ...
    async with search_api(roles=[NucliaDBRoles.READER]) as client:
        await asyncio.sleep(1)
        resp = await client.get(
            f"/{KB_PREFIX}/{kbid}/find?query=own+text&split=true&highlight=true&text_resource=true",
        )
        assert resp.status_code == 200
