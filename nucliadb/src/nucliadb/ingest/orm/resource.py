...
class Resource:
    ...
    async def apply_da_cleanup(self, cleanup: writer_pb2.DataAugmentationCleanup):
        da_task_id = cleanup.task_id

        # Remove text fields that were generated by the data augmentation task
        for field in cleanup.text_fields:
            self.modified = True
            field_obj = await self.get_field(field, FieldType.TEXT, load=True)
            generated_by = await field_obj.generated_by()
            if generated_by.WhichOneof("author") == "data_augmentation":
                await self.delete_field(FieldType.TEXT, key=field)

        cleanup_labels = cleanup.HasField("labels")
        if cleanup_labels or cleanup.qas or cleanup.entities:
            fields = await self.get_fields(force=True)
            for (ftype, fkey), field_obj in fields.items():
                if cleanup.qas:
                    self.modified = True
                    await field_obj.delete_question_answers()

                if cleanup_labels or cleanup.entities:
                    fcm = await field_obj.get_field_metadata()
                    if fcm is not None:
                        if cleanup_labels:
                            if cleanup.labels.fields:
                                # Remove all field-level classifications that belong to the labelsets to be cleaned up
                                clean_classifications = [
                                    classification
                                    for classification in fcm.metadata.classifications
                                    if classification.labelset in cleanup.labels.labelsets
                                ]
                                fcm.metadata.ClearField("classifications")
                                fcm.metadata.classifications.extend(clean_classifications)

                            if cleanup.labels.paragraphs:

                                def _clean_paragraph_classifications(fcm, cleanup):
                                    # Remove all paragraph-level classifications that belong to the labelsets to be cleaned up
                                    for paragraph in fcm.metadata.paragraphs:
                                        ...
                                        if len(clean_classifications) != len(paragraph.classifications):
                                            paragraph.ClearField("classifications")
                                            paragraph.classifications.extend(clean_classifications)

                                # As there can be many paragraphs, we run this in an executor
                                loop = asyncio.get_event_loop()
                                await loop.run_in_executor(
                                    _executor, partial(_clean_paragraph_classifications, fcm, cleanup)
                                )
        ...
        if cleanup.relations:
            # Remove all relations that belong to the task to be cleaned up
