...
async def test_get_set_all_fields_key(txn, storage, kb):
    resource = Resource(txn, storage, kb, "rid")
    ...
    assert await resource.get_all_fields_key() == all_fields


async def test_update_all_fields_key(txn, storage, kb):
    resource = Resource(txn, storage, kb, "rid")

    await resource.update_all_fields_key(updated=[], deleted=[])
    ...
    await resource.update_all_fields_key(updated=all_fields.fields)

    # Check updates
    assert await resource.get_all_fields_key() == all_fields

    file_field = FieldID(field_type=FieldType.FILE, field="file")
    await resource.update_all_fields_key(updated=[file_field])

    result = await resource.get_all_fields_key()
    assert list(result.fields) == list(all_fields.fields) + [file_field]

    # Check deletes
    await resource.update_all_fields_key(deleted=[file_field])

    assert await resource.get_all_fields_key() == all_fields


async def test_apply_fields(txn, storage, kb):
    resource = Resource(txn, storage, kb, "rid")
    resource.update_all_fields_key = AsyncMock()  # type: ignore
    ...
    resource.update_all_fields_key.assert_awaited_once()

    resource.update_all_fields_key.call_args[1]["updated"] == [
    ...
    resource.update_all_fields_key.call_args[1]["deleted"] == [
