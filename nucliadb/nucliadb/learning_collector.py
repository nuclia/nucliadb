...
async def proxy(
    ...
    """
    Proxy the request to the learning collector API.

    request: Request. The incoming request.
    method: str. The HTTP method to use.
    url: str. The URL to proxy the request to.

    Returns: Response. The response from the learning collector API.
    ...
    async with learning_collector_client() as client:
        try:
            ...
            return Response(
                content=b"Unexpected error while trying to proxy the request to the learning collector API. Please try again later.",  # noqa
                status_code=503,
...
async def learning_collector_client() -> AsyncIterator[httpx.AsyncClient]:
    """
    ...
    if nuclia_settings.dummy_learning_collector:
        # This is a workaround to be able to run integration tests that start nucliadb with docker.
        # The learning collector API is not available in the docker setup, so we use a dummy client.
        client_class = DummyClient
        logger.warning(
            "Using dummy learning collector client. If you see this in production, something is wrong."
        )
    else:
