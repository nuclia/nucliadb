...
In general what we expect from a given stratification output is that a strata, or a fold, is
close to a given, demanded size,
usually equal to 1/k in k-fold approach, or a x% train to test set division in 2-fold splits.

The idea behind this stratification method is to assign label combinations to folds based on how
much a given combination
is desired
by a given fold, as more and more assignments are made, some folds are filled and positive evidence is
directed into other
folds,
in the end negative evidence is distributed based on a folds desirability of size.

You can also watch a `video presentation
<http://videolectures.net/ecmlpkdd2011_tsoumakas_stratification/?q=stratification%20multi%20label>`_ by G.
Tsoumakas which explains the algorithm. In 2017 Szymanski & Kajdanowicz extended the algorithm
to handle high-order relationships in the data set, if order = 1, the algorithm falls back to the original
Sechidis11 setting.

If order is larger than 1 this class constructs a list of label combinations with replacement, i.e.
allowing combinations of lower
...
Sechidis, K., Tsoumakas, G., & Vlahavas, I. (2011). On the stratification of multi-label data. Machine Learning and
Knowledge Discovery in Databases, 145-158.
http://lpis.csd.auth.gr/publications/sechidis-ecmlpkdd-2011.pdf

Piotr Szymański, Tomasz Kajdanowicz ; Proceedings of the First International Workshop on Learning with
Imbalanced Domains: Theory and Applications, PMLR 74:22-35, 2017.
...
.. code-block:: bibtex
    ...
    @InProceedings{pmlr-v74-szymański17a,
      ...
      booktitle =    {Proceedings of the First International Workshop on Learning with Imbalanced Domains:
...
class IterativeStratification(_BaseKFold):
    ...
    def __init__(
        self, n_splits=3, order=1, sample_distribution_per_fold=None, random_state=None
    ):
        self.order = order
    ...
    def _prepare_stratification(self, y):
        ...
        self.desired_samples_per_combination_per_fold: Dict[Combination, np.array[Float]],
            :code:`(n_combinations, n_splits)`
    ...
    def _distribute_positive_evidence(
        ...
        modifies params.
        """
        l = _get_most_desired_combination(samples_with_combination)  # noqa
        while l is not None:
