...
@pytest.mark.asyncio
async def test_empty_paragraph_annotations_are_ignored_on_serialize(
    nucliadb_reader: AsyncClient,
    nucliadb_grpc: WriterStub,
    knowledgebox: str,
):
    """
    Test description:
    - Add a broker message with a paragraph annotation that has no classifications
    - Check that the resource serializes correctly
    - Check that the paragraph annotation is not returned
    """

    br = broker_resource(knowledgebox)

    field = rpb.FieldID(field_type=rpb.FieldType.TEXT, field="text1")
    br.texts[field.field].body = "Some text"

    ufm = rpb.UserFieldMetadata()
    ufm.field.CopyFrom(field)
    pa = rpb.ParagraphAnnotation(key="foo")
    assert len(pa.classifications) == 0
    ufm.paragraphs.append(pa)
    br.basic.fieldmetadata.append(ufm)
    ...
    # Check that the paragraph annotation has been skipped
    assert resource["fieldmetadata"][0]["paragraphs"] == []
