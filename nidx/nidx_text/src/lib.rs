...
use nidx_tantivy::{TantivyIndexer, TantivySegmentMetadata};
...
use tantivy::{Directory, IndexSettings, SegmentMeta, SegmentReader};
use tantivy_common::{BitSet, ReadOnlyBitSet, TerminatingWrite};
...
impl TextIndexer {
    ...
    ) -> NodeResult<TantivySegmentMetadata> {
    ...
    pub fn merge(
        ...
        let output_index = merge_indices(&[index], MmapDirectory::open(work_dir.join("output"))?)?;
...
mod tests {
    ...
    use crate::{TantivySegmentMetadata, TextIndexer};

    #[test]
    fn test_merge() -> NodeResult<()> {
        ...
        let merged_records = TextIndexer.merge(
            merge_dir.path(),
            vec![(meta1, 1i64.into()), (meta2, 2i64.into())],
            &[(2i64.into(), &vec![resource.resource.unwrap().uuid])],
        )?;

        assert_eq!(merged_records, records * 2);
