...
pub fn search_query(
    ...
    if let Some(security) = search.security.as_ref() {
    ...
    if !access_groups_queries.is_empty() {
        let public_fields_query =
            Box::new(TermQuery::new(Term::from_field_u64(schema.groups_public, 1_u64), IndexRecordOption::Basic));
        ...
        fuzzies.push((Occur::Must, access_groups_query.box_clone()));
        originals.push((Occur::Must, access_groups_query));
    }
