...
const INVERSE_KEYS_DB: &str = "INVERSE_KEYS_LMDB";
const EDGE_DB: &str = "EDGE_LMDB";
const INVERSE_EDGE_DB: &str = "INVERSE_EDGE_LMDB";
...
impl StorageSystem {
    pub fn create(path: &Path) -> StorageSystem {
        ...
        if !env_path.exists() {
            ...
            let inv_keys = env.create_database(Some(INVERSE_KEYS_DB)).unwrap();
            let edges = env.create_database(Some(EDGE_DB)).unwrap();
            let in_edges = env.create_database(Some(INVERSE_EDGE_DB)).unwrap();
    ...
    pub fn open(path: &Path) -> StorageSystem {
        ...
        let inv_keys = env.open_database(Some(INVERSE_KEYS_DB)).unwrap().unwrap();
        let edges = env.open_database(Some(EDGE_DB)).unwrap().unwrap();
        let in_edges = env.open_database(Some(INVERSE_EDGE_DB)).unwrap().unwrap();
    ...
    fn get_fresh_node_id(&self, txn: &mut RwTxn) -> NodeId {
        let mut fresh = self
            .state
            .get(txn, storage_state::FRESH_NODE)
            .unwrap()
            .map(NodeId::from_byte_rpr)
            .unwrap_or_else(NodeId::new);
        let current = fresh.next();
        self.state
            .put(txn, storage_state::FRESH_NODE, &fresh.as_byte_rpr())
            .unwrap();
        current
    }
