...
fn default_storage() -> HashMap<String, usize> {
    HashMap::from([("".to_string(), 0)])
}
...
impl Query {
    ...
    fn type_qualifies(&self, node: &Node) -> bool {
        ...
        self.types_allowed.is_empty()
            ...
            || mix
                .map(|(t, _)| (t, self.wildcard_type))
                .map_or(false, |v| self.types_allowed.contains(&v))
    }
    fn prefix_qualifies(&self, node: &Node) -> bool {
        node.get_value().starts_with(&self.prefixed)
    }
...
fn standard_query(entry_points: &[NodeId], storage: &StorageSystem, query: Query) -> QueryMatches {
    ...
    while let Some(point) = work.pop_back() {
        ...
        if point.depth < stop_at {
            for value in point.expand(&txn, storage, &visited) {
...
mod tests {
    ...
    lazy_static::lazy_static! {
        pub static ref NA: Node = Node::from(r#"{ "value": "Book_A", "type": "other", "subtype": "" }"#);
        pub static ref NB: Node = Node::from(r#"{ "value": "Book_B", "type": "other", "subtype": "" }"#);
        pub static ref NC: Node = Node::from(r#"{ "value": "Actor_C","type": "other", "subtype": "person" }"#);
        pub static ref ND: Node = Node::from(r#"{ "value": "Film_D", "type": "other", "subtype": "" }"#);
    }
