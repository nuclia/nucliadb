...
fn simple_flow() {
    ...
    let reader = data_point::create(&pin, elems, None, &CONFIG, vec![]).unwrap();
...
fn single_graph() {
    ...
    let pin = DataPointPin::create_pin(temp_dir.path()).unwrap();
    let reader = data_point::create(&pin, elems.clone(), None, &CONFIG, vec![]).unwrap();
...
fn data_merge() {
    ...
    let dp0 = data_point::create(&dp0_pin, elems0, None, &CONFIG, vec![]).unwrap();

    let dp1_pin = DataPointPin::create_pin(temp_dir.path()).unwrap();
    let dp1 = data_point::create(&dp1_pin, elems1, None, &CONFIG, vec![]).unwrap();
...
fn fast_data_merge() -> VectorR<()> {
    ...
    let big_segment = data_point::create(&big_segment_pin, elems, None, &CONFIG, vec![])?;
