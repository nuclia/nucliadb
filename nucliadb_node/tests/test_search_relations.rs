...
use common::{node_reader_client, node_writer_client, TestNodeWriter};
...
use nucliadb_protos::{
    ...
    RelationNode,
    Resource,
    ResourceId, ShardId, RelationFilter, RelationPrefixSearchRequest, RelationNeighboursSearchRequest,
};
...
async fn test_search_relations_prefixed() -> Result<(), Box<dyn std::error::Error>> {
    let mut reader = node_reader_client().await;
...
async fn test_search_relations_neighbours() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = reader.relation_search(
        RelationSearchRequest {
            ...
            neighbours: Some(RelationNeighboursSearchRequest {
                entry_points: vec![
                    EntryPoint {
                        node: Some(relation_nodes.get("Swallow").unwrap().clone()),
                        depth: 1,
                    }
                ],
                type_filters: vec![
                    // RelationFilter {ntype: NodeType::User as i32, subtype: "".to_string()},
                ],
            }),
            ..Default::default()
        }
    ...
    let neighbours = response.get_ref().neighbours
        .iter()
        .flat_map(|neighbour| neighbour.neighbours.iter() )
