...
async def test_migration_0051(maindb_driver: Driver):
    ...
    async with maindb_driver.rw_transaction() as txn:
        find: SearchConfiguration = FindSearchConfiguration(
            kind="find", config=FindConfig(generative_model="nova-premier")
        )
        ...
        ask: SearchConfiguration = AskSearchConfiguration(
            kind="ask", config=AskConfig(generative_model="nova-premier")
        )
    ...
    async with maindb_driver.ro_transaction() as txn:
        ...
        assert find_.config.generative_model == "nova-pro"  # type: ignore[attr-defined]

        ask_ = await datamanagers.search_configurations.get(txn, kbid=kbid, name="ask")
        assert ask_ is not None
        assert ask_.config.generative_model == "nova-pro"  # type: ignore[attr-defined]
